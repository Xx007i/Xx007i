...
(function() {
    ...
    g.Snake = function(opt) {
        ...
        // sync data grid of the play state(同步数据网格的播放状态)
    ...
    g.Snake.prototype.update = function() {
        ...
        if (this.updateTick >= this.updateTickMax) {
            // reset the update timer to 0, or whatever leftover there is(将更新计时器重置为0，或任何剩余的值)
            this.updateTick = this.updateTick - this.updateTickMax;

            // rotate snake block array(旋转蛇块阵列)
            this.tiles.unshift(
                new g.SnakeTile({
                    parentState: this.parentState,
                    parentGroup: this.tiles,
            ...
            // check death by eating self(通过吃自己来控制死亡)
            if (
    ...
    g.Snake.prototype.render = function() {
        // render individual snake(渲染单独的蛇)
        var i = this.tiles.length;
        while (i--) {
...
(function() {
    ...
    g.Food.prototype.update = function() {
        ...
        // sync data grid of the play state(同步播放状态的数据网格)
